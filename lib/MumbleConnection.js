 ...
 **/
MumbleConnection.prototype.outputStream = function () {
...
MumbleConnection.prototype._processMessage = function( type, msg ) {
    ...
    this.emit( 'protocol-in', { type: Messages.nameById[ type ], message: msg });
};
...
/**
 * Handle user remove message
 *
 * @param userState User state message
 */
MumbleConnection.prototype._onUserRemove = function ( userRemove ) {

    var user = this.users[ userRemove.session ];
	/*
    if( !user ) {
        user = this.users[ userState.session ] = {
            talking: false,
...
MumbleConnection.prototype._processAudio = function() {
    ...
    while( this.lastProcess + this.FRAME_LENGTH < Date.now() )
    {
        ...
        if( packets.length > 0 && (
            ...
            this.listeners('voice-premix').length > 0 )) {
            ...
            this.emit('voice-premix', decoded);
